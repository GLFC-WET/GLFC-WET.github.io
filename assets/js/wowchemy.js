...
function scrollToAnchor(target, duration = 0) {
  ...
  if ($(target).length) {
    ...
    let elementOffset = Math.ceil($(target).offset().top); // Round up to highlight right ID!
...
$('#navbar-main li.nav-item a.nav-link, .js-scroll').on('click', function (event) {
  ...
  if (this.pathname === window.location.pathname && hash && $(hash).length && $('.js-widget-page').length > 0) {
    ...
    let elementOffset = Math.ceil($(hash).offset().top); // Round up to highlight right ID!

    // Uncomment to debug.
    // let scrollTop = $(window).scrollTop();
    // let scrollDelta = (elementOffset - scrollTop);
    // console.debug('Scroll Delta: ' + scrollDelta);F
